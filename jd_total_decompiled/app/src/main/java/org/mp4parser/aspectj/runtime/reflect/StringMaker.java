...
class StringMaker {
    ...
    String makeTypeName(Class cls, String str, boolean z) {
        ...
        if (!cls.isArray()) {
            if (z) {
                return stripPackageName(str).replace(Typography.dollar, OrderISVUtil.MONEY_DECIMAL_CHAR);
            }
            return str.replace(Typography.dollar, OrderISVUtil.MONEY_DECIMAL_CHAR);
        }
