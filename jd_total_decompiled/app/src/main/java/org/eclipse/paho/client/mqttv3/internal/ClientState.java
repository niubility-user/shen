...
public class ClientState {
    ...
    /* JADX WARN: Code restructure failed: missing block: B:126:0x0095, code lost:
        if ((r4 - r16.lastOutboundActivity) >= (r16.keepAlive * 2)) goto L128;
     */
    ...
    public MqttToken checkForActivity(IMqttActionListener iMqttActionListener) throws MqttException {
        ...
        synchronized (this.quiesceLock) {
            ...
            synchronized (this.pingOutstandingLock) {
                ...
                if (i2 > 0 && currentTimeMillis - this.lastInboundActivity >= this.keepAlive + 100) {
                    logger.severe(str, "checkForActivity", "619", new Object[]{new Long(this.keepAlive), new Long(this.lastOutboundActivity), new Long(this.lastInboundActivity), new Long(currentTimeMillis), new Long(this.lastPing)});
                    throw ExceptionHelper.createMqttException(32000);
                }
                if ((i2 == 0 && currentTimeMillis - this.lastInboundActivity >= this.keepAlive - 100) || currentTimeMillis - this.lastOutboundActivity >= this.keepAlive - 100) {
