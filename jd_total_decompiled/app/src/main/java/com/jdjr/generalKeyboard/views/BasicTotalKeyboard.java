...
public class BasicTotalKeyboard extends KeyboardView implements View.OnTouchListener, View.OnClickListener, View.OnLongClickListener {
    ...
    */
    private void drawKeyText(ViewGroup viewGroup, int i2) {
        String[][] strArr;
        int i3;
        String[][] strArr2;
        if (i2 != 1) {
            if (i2 == 2) {
                strArr2 = this.letterTexts;
                i3 = this.mKeyViewWidth;
                this.baseline = (this.mKeyViewHeight * 6) / 10;
            } else if (i2 != 3) {
                strArr2 = null;
                i3 = 0;
            } else {
                strArr = this.SYMBOL_TEXTS_TYPE == 1 ? this.symbolTextsMore1 : this.symbolTextsMore2;
                i3 = this.mKeyViewWidth;
                double d = this.mKeyViewHeight;
                Double.isNaN(d);
                        ...
                        if (childAt2 != null && (childAt2 instanceof ViewGroup)) {
                            ...
                            if (str != null && !str.equals("none") && (childAt3 instanceof TotalKeyView)) {
                                ...
                                if (!totalKeyView.getEnlargePopType().contains("_normal")) {
                                    ...
                                    totalKeyView.setWidth(i3);
                                }
                                totalKeyView.setBaseline(this.baseline);
                                totalKeyView.setKeyboardType(i2);
                                totalKeyView.setTextColor(this.keyViewTextColor);
                                childAt3.setBackgroundResource(this.keyViewBackground);
                                new Handler().postDelayed(new Runnable() { // from class: com.jdjr.generalKeyboard.views.BasicTotalKeyboard.1
                                    @Override // java.lang.Runnable
                                    public void run() {
                                        BasicTotalKeyboard.this.saveKeyViewRect(childAt3);
                                    }
                                }, 500L);
                            }
                        }
        ...
        strArr = this.numberTexts;
        i3 = this.mKeyViewWidth;
        double d2 = this.mKeyViewHeight;
        Double.isNaN(d2);
