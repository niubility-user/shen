...
public abstract class MultiTypesAdapter<T extends TypeIdentifier, VH extends RecyclerView.ViewHolder> extends RecyclerView.Adapter<VH> {
    ...
    */
    public void filterModelsNew(@Nullable List<? extends T> models) {
        AdapterDelegatesManager<T, VH> adapterDelegatesManager;
        List mutableList;
        ListIterator listIterator = (models == null || mutableList == null) ? null : mutableList.listIterator();
        if (listIterator != null) {
            while (listIterator.hasNext()) {
                TypeIdentifier typeIdentifier = (TypeIdentifier) listIterator.next();
                if (typeIdentifier == null || ((adapterDelegatesManager = this.delegatesManager) != 0 && adapterDelegatesManager.getViewType(typeIdentifier) == -1)) {
                    listIterator.remove();
                }
            }
        }
