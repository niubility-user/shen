...
public class FlexCubeLayerHelper {
    ...
    */
    public int dealSize(FlexCubeModel flexCubeModel, XViewConfigEntity.LayersEntity layersEntity) {
        boolean z;
        double ceil;
        int i2;
        XViewWebviewEntity xViewWebviewEntity;
        XViewConfigEntity.LayoutEntity layoutEntity;
        int i3 = layersEntity.layout.screenWidth;
        if (flexCubeModel.floorConfig != null && i3 > 0) {
            float appWidth = (DPIUtil.getAppWidth((Activity) this.mContext) * 1.0f) / i3;
            if ("1".equals(layersEntity.layout.fill) && (layoutEntity = layersEntity.renderData.layout) != null && "1".equals(layoutEntity.contentFillMode)) {
                ...
                if (floorConfig.w > 0 && floorConfig.f4266h > 0) {
                    ...
                    if (!z) {
                        ...
                        int round = Math.round(floorConfig2.f4266h * ((DPIUtil.getAppWidth((Activity) this.mContext) * 1.0f) / floorConfig2.w));
                        XView2Container xView2Container = this.mFlexCubeLayer.mXViewContainer;
                        int height = xView2Container != null ? xView2Container.getHeight() : 0;
                        if (height <= 0) {
                            ScreenSize realScreenSize = BaseInfo.getRealScreenSize();
                            height = realScreenSize != null ? realScreenSize.heightPixels : 0;
                        }
                        if (round < height && this.xViewWebviewEntity == null) {
                            FloorConfig floorConfig3 = flexCubeModel.floorConfig;
                            ceil = Math.ceil(floorConfig3.w * ((height * 1.0f) / floorConfig3.f4266h));
                        } else {
                            int appWidth2 = DPIUtil.getAppWidth((Activity) this.mContext);
                            if (this.xViewWebviewEntity != null) {
                                flexCubeModel.floorConfig.f4266h = (int) Math.ceil(height / r7);
                                CanvasConfig canvasConfig = flexCubeModel.canvasConfig;
                                if (canvasConfig != null) {
                                    canvasConfig.f4223h = flexCubeModel.floorConfig.f4266h;
                                }
                            }
                            i2 = appWidth2;
                            r2 = i2;
                            xViewWebviewEntity = this.xViewWebviewEntity;
                            if (xViewWebviewEntity != null) {
                                xViewWebviewEntity.getBaseConfig().x = 0.0f;
                                this.xViewWebviewEntity.getBaseConfig().y = 0.0f;
                                this.xViewWebviewEntity.getBaseConfig().w = flexCubeModel.canvasConfig != null ? r0.w : flexCubeModel.floorConfig.w;
                                this.xViewWebviewEntity.getBaseConfig().f4225h = flexCubeModel.canvasConfig != null ? r0.f4223h : flexCubeModel.floorConfig.f4266h;
                            }
                        }
                    } else {
                        ceil = Math.ceil(flexCubeModel.floorConfig.w * appWidth);
                    }
                    i2 = (int) ceil;
                    r2 = i2;
                    xViewWebviewEntity = this.xViewWebviewEntity;
