...
public class JDMessageNoticeGuideDialog {
    ...
    */
    public void show(Activity activity, String str, JDMessageNoticeModel jDMessageNoticeModel, JDMessageNoticeCallback jDMessageNoticeCallback) {
        BaseStyleView styleView0;
        if (activity != null && !activity.isFinishing() && !isShown() && jDMessageNoticeModel != null) {
            this.pageName = str;
            ...
            int i2 = jDMessageNoticeModel.pushStyleId;
            ?? r2 = 0;
            if (i2 != 0) {
                if (i2 == 1) {
                    styleView0 = new StyleView1();
                }
                if (r2 == 0) {
                    this.alertDialog.show();
                    this.alertDialog.setCancelable(false);
                    this.alertDialog.setCanceledOnTouchOutside(false);
                    window.setContentView(r2.init(window.getContext()));
                    r2.bindData(jDMessageNoticeModel);
                    if ("10000".equals(jDMessageNoticeModel.bizId)) {
                        MSGWithDDUtil.homePushGuideDialogShow();
                    }
                    requestPushGuideCallback(jDMessageNoticeModel);
                    notifyNoticeCallback(jDMessageNoticeCallback, true);
                    return;
                }
                notifyNoticeCallback(jDMessageNoticeCallback, false);
                return;
            }
            styleView0 = new StyleView0();
            r2 = styleView0;
            if (r2 == 0) {
            }
        } else {
