...
public class AppRTCBluetoothManager {
    ...
    */
    public void bluetoothTimeout() {
        ...
        OKLog.d(TAG, "bluetoothTimeout: BT state=" + this.bluetoothState + ", attempts: " + this.scoConnectionAttempts + ", SCO is on: " + isScoOn());
        ...
        if (connectedDevices.size() > 0) {
            ...
            if (this.bluetoothHeadset.isAudioConnected(bluetoothDevice)) {
                OKLog.d(TAG, "SCO connected with " + this.bluetoothDevice.getName());
                z = true;
                if (!z) {
                    this.bluetoothState = State.SCO_CONNECTED;
                    this.scoConnectionAttempts = 0;
                } else {
                    OKLog.w(TAG, "BT failed to connect after timeout");
                    stopScoAudio();
                }
                updateAudioDeviceState();
                OKLog.d(TAG, "bluetoothTimeout done: BT state=" + this.bluetoothState);
            }
            OKLog.d(TAG, "SCO is not connected with " + this.bluetoothDevice.getName());
        }
        z = false;
        if (!z) {
        }
