...
public class UnStatusBarTintHelper {
    ...
    private static OnStatusBarListener controller;
    private static int mStatuBarType;
    private static Integer sTransparentValue;

    public UnStatusBarTintHelper(@NonNull OnStatusBarListener onStatusBarListener) {
        controller = onStatusBarListener;
    }

    @TargetApi(23)
    private boolean Android6SetStatusBarLightMode(Window window, boolean z) {
    ...
    private void addStatusBarView(Activity activity, View view, boolean z) {
    ...
    public static boolean isEnable(Activity activity) {
        ...
        OnStatusBarListener onStatusBarListener2;
        ...
        return (greaterOrEqualN() && controller.isMultiWindowMode() && (onStatusBarListener2 = controller) != null) ? onStatusBarListener2.statusBarHint() == 1 : !UnAndroidUtils.mateXEasyClient(activity) || (onStatusBarListener = controller) == null || onStatusBarListener.statusBarHint() == 1;
    }
    ...
    private static boolean specialPhoneUnable(Context context) {
        ...
        if (UnAndroidUtils.isTabletDevice(context) && TextUtils.equals("HUAWEI", UnDeviceInfo.getDeviceModel())) {
    ...
    public void cutout(Activity activity) {
        boolean isDisplayCutoutLocal;
        try {
            OnStatusBarListener onStatusBarListener = controller;
            if (onStatusBarListener != null) {
                isDisplayCutoutLocal = onStatusBarListener.isDisplayCutout();
            } else {
    ...
    public void defaultSetStatusBarbg(Activity activity) {
    ...
    public void defaultSetTranslucent(Activity activity) {
        if (isEnable(activity)) {
            View statusBarView = getStatusBarView(activity);
            if (UnLog.D) {
    ...
    public View getStatusBar(Activity activity) {
        return getDecorView(activity).findViewById(R.id.un_status_bar_view);
    }

    public Integer getStatusBarAPITransparentValue(Context context) {
    ...
    public void newDefaultSetStatusBarbg(Activity activity) {
        if (isEnable(activity)) {
            View statusBarView = getStatusBarView(activity);
            if (UnLog.D) {
                UnLog.d("statusbar-sencond", "no tran--view-->" + statusBarView);
    ...
    public void refresh(Activity activity, boolean z) {
        if (z) {
            ...
            setBackgroundResource(activity, R.color.status_bar_bg_dark);
    ...
    public void removeStatusView(Activity activity) {
        ViewGroup decorView = getDecorView(activity);
        View findViewById = decorView.findViewById(R.id.un_status_bar_view);
        if (findViewById != null) {
            decorView.removeView(findViewById);
    ...
    public int retainSystemUiFlag(Window window, int i2, int i3) {
    ...
    public void setBackgroundColor(Activity activity, int i2) {
        View statusBarView;
    ...
    public void setBackgroundResource(Activity activity, int i2) {
        View statusBarView;
    ...
    public void setDefaultBg(Activity activity, boolean z) {
        if (isEnable(activity)) {
            if (z) {
                setBackgroundColor(activity, 0);
    ...
    public void setFitsSystemWindows(Activity activity, int i2) {
    ...
    public boolean setLightOrDarkEnable(Activity activity) {
    ...
    public void setStatusBar4Base(Activity activity, int i2) {
    ...
    public boolean setStatusBarDarkMode(Activity activity) {
        if (activity == null) {
            return false;
        }
        if (mStatuBarType == 0) {
    ...
    public boolean setStatusBarLightMode(Activity activity) {
        if (setLightOrDarkEnable(activity)) {
            int i2 = mStatuBarType;
            if (i2 != 0) {
    ...
    public void setStatusBar4Base(Activity activity, int i2, boolean z) {
        if (activity == null || !greaterOrEqualKitkat()) {
            return;
        }
        if (i2 == 0) {
    ...
    public void setDefaultBg(Activity activity, boolean z, boolean z2, boolean z3) {
        if (isEnable(activity)) {
            if (z) {
                setBackgroundColor(activity, 0);
    ...
    public boolean setLightOrDarkEnable(Context context) {
    ...
    private boolean setStatusBarLightMode(Activity activity, int i2) {
        if (i2 == 1) {
    ...
    public static boolean isEnable(Context context) {
        ...
        if (context instanceof Activity) {
            ...
            if (greaterOrEqualN() && activity.isInMultiWindowMode() && (onStatusBarListener2 = controller) != null) {
                return onStatusBarListener2.statusBarHint() == 1;
            }
        }
        return !UnAndroidUtils.mateXEasyClient(context) || (onStatusBarListener = controller) == null || onStatusBarListener.statusBarHint() == 1;
    }

    public void defaultSetStatusBarbg(Activity activity, boolean z) {
        if (isEnable(activity)) {
            View statusBarView = getStatusBarView(activity);
            if (UnLog.D) {
                UnLog.d("statusbar-sencond", "no tran--view-->" + statusBarView);
    ...
    public void newDefaultSetStatusBarbg(Activity activity, boolean z) {
        if (isEnable(activity)) {
            View statusBarView = getStatusBarView(activity);
            if (UnLog.D) {
                UnLog.d("statusbar-sencond", "no tran--view-->" + statusBarView);
