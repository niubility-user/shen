...
public class MiniProgramSearchHistoryTable implements IJdTable {
    ...
    */
    private static MiniProgramSearchHistory getSearchHistoryFromText(SQLiteDatabase sQLiteDatabase, String str, int i2) {
        Cursor cursor;
        ?? r6 = 0;
                ...
                if (cursor != null) {
                    try {
                        if (cursor.moveToFirst()) {
                            MiniProgramSearchHistory miniProgramSearchHistory = new MiniProgramSearchHistory(cursor.getInt(cursor.getColumnIndex("_id")), cursor.getString(cursor.getColumnIndex("word")), cursor.getString(cursor.getColumnIndex("tag")), cursor.getLong(cursor.getColumnIndex("search_time")), cursor.getInt(cursor.getColumnIndex("type")));
                            if (cursor != null && !cursor.isClosed()) {
                                cursor.close();
                            }
                            return miniProgramSearchHistory;
                        }
                    } catch (Exception e2) {
                        e = e2;
                        if (OKLog.E) {
    ...
    /*
        Code decompiled incorrectly, please refer to instructions dump.
    */
    private static ArrayList<MiniProgramSearchHistory> getAllSearchHistory(SQLiteDatabase sQLiteDatabase) {
        Throwable th;
        Cursor cursor;
        ArrayList<MiniProgramSearchHistory> arrayList;
        Exception e2;
        try {
            ...
            try {
                try {
                    ...
                    while (cursor.moveToNext()) {
                        try {
                            arrayList.add(new MiniProgramSearchHistory(cursor.getInt(cursor.getColumnIndex("_id")), cursor.getString(cursor.getColumnIndex("word")), cursor.getString(cursor.getColumnIndex("tag")), cursor.getLong(cursor.getColumnIndex("search_time")), cursor.getInt(cursor.getColumnIndex("type"))));
                        } catch (Exception e3) {
                            e2 = e3;
                            if (OKLog.E) {
                                OKLog.e(TAG, e2);
