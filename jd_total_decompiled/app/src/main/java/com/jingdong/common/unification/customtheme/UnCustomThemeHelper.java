...
public class UnCustomThemeHelper {
    ...
    /* JADX WARN: Removed duplicated region for block: B:388:0x01df  */
    /* JADX WARN: Removed duplicated region for block: B:390:0x01e6  */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
    */
    public void dealResponseData(String str, boolean z) {
        String str2;
        JDJSONObject jSONObject;
        HashMap<String, List<ImageInfoEntity>> hashMap;
        JDJSONObject parseObject = JDJSON.parseObject(str);
        boolean optBoolean = parseObject.optBoolean("isOpen");
        String optString = parseObject.optString("skinId");
        this.version = TextUtils.isEmpty(parseObject.optString("version")) ? "-100" : parseObject.optString("version");
        if (TextUtils.isEmpty(optString)) {
            optString = "0";
        }
        if (!optBoolean) {
            if (TextUtils.equals(optString, getCurThemeId()) && !TextUtils.equals(getCurThemeId(), "0") && !TextUtils.isEmpty(getCurThemeId())) {
                this.workHandler.removeMessages(1);
                this.workHandler.removeMessages(2);
                requestData("0");
                return;
            }
            notifyDownloadThemeResult(optString, false);
            return;
        }
        if (!parseObject.optBoolean("forwardDegrade")) {
            String optString2 = parseObject.optString("verifyCode");
            String optString3 = parseObject.optString(ThemeTitleConstant.TITLE_FORWARD_DRAWABLE_ID);
            long optLong = parseObject.optLong(DYConstants.DY_COUNTDOWN);
            if (!TextUtils.isEmpty(optString3) && optLong > 0) {
                if (optLong > 0) {
                    countDownFresh(optLong);
                }
                if (!TextUtils.isEmpty(optString2)) {
                    if (!optString2.equals(getVerifyCode())) {
                        setVerifyCode(optString2);
                        writeJson(optString3);
                    }
                    JDJSONObject parseObject2 = JDJSON.parseObject(optString3);
                    String optString4 = parseObject2.optString("skinId");
                    JDJSONObject jSONObject2 = parseObject2.getJSONObject("modeMap");
                    if (jSONObject2 != null) {
                        HashMap<String, ImageInfoEntity> parseImageData = parseImageData(jSONObject2);
                        HashMap<String, List<ImageInfoEntity>> dataCategory = dataCategory(parseImageData);
                        Set<String> keySet = dataCategory.keySet();
                        List<File> readLocalFile = readLocalFile(CustomThemeConstance.SAVE_DIR_FOR_PIC);
                        boolean z2 = readLocalFile == null || readLocalFile.size() == 0;
                        HashMap<String, List<ImageInfoEntity>> hashMap2 = new HashMap<>();
                        if (z2) {
                            hashMap2.putAll(dataCategory);
                            str2 = optString;
                        } else {
                            ...
                            while (it.hasNext()) {
                                ...
                                String str4 = creatFilePath(CustomThemeConstance.SAVE_DIR_FOR_PIC) + "/" + optString4 + CartConstant.KEY_YB_INFO_LINK + creatFileName(next);
                                if (UnLog.D) {
                                    UnLog.d("custom_theme", "compare filePath " + str4);
                                }
                                File file = new File(str4);
                                if (!file.exists()) {
                                    hashMap2.put(next, dataCategory.get(next));
                                } else {
                                    Iterator<ImageInfoEntity> it3 = dataCategory.get(next).iterator();
                                    while (it3.hasNext()) {
                                        Iterator<ImageInfoEntity> it4 = it3;
                                        ImageInfoEntity next2 = it3.next();
                                        next2.localPath = str4;
                                        parseImageData.put(next2.imageId, next2);
                                        str4 = str4;
                                        it3 = it4;
                                    }
                                    try {
                            ...
                            str2 = optString;
                            deletePics(readLocalFile);
                        }
                        if (hashMap2.size() > 0) {
                            prepareDownload(optString4, hashMap2);
                        }
                    }
                } else {
                    str2 = optString;
            ...
            this.navigationIds = parseObject.optString("navigationOrder");
            OKLog.e("tianchuangxin1", "--navigationIds->" + this.navigationIds);
            this.skinType = parseObject.optInt("type");
            this.navigationMtaParams = parseObject.optString("paramValues");
            this.currentMode = parseObject.optString("currentMode");
            jSONObject = parseObject.getJSONObject("modeMap");
            if (jSONObject != null) {
                notifyDownloadThemeResult(str2, false);
                return;
            }
            String str5 = str2;
            JDJSONObject jSONObject3 = jSONObject.getJSONObject("normal");
            if (jSONObject3 == null) {
                notifyDownloadThemeResult(str5, false);
                return;
            }
            this.navigationInfos = parseNaviData(jSONObject3, false);
            JDJSONObject jSONObject4 = jSONObject.getJSONObject(CustomThemeConstance.NAVI_IMAGE_DARK_TAG);
            if (this.navigationInfos != null && parseNaviData(jSONObject4, true) != null) {
                this.navigationInfos.addAll(parseNaviData(jSONObject4, true));
            }
            HashMap<String, ImageInfoEntity> parseImageData2 = parseImageData(jSONObject);
            if (parseImageData2 != null && !parseImageData2.isEmpty()) {
                HashMap<String, List<ImageInfoEntity>> dataCategory2 = dataCategory(parseImageData2);
                Set<String> keySet2 = dataCategory2.keySet();
                ...
                List<File> readLocalFile2 = readLocalFile(CustomThemeConstance.SAVE_DIR);
                boolean z3 = readLocalFile2 == null || readLocalFile2.size() == 0;
                HashMap<String, List<ImageInfoEntity>> hashMap3 = new HashMap<>();
                this.deleteImageFile.clear();
                if (z3) {
                    hashMap3.putAll(dataCategory2);
                } else {
                    for (String str6 : keySet2) {
                        String str7 = creatFilePath(CustomThemeConstance.SAVE_DIR) + "/" + str5 + CartConstant.KEY_YB_INFO_LINK + creatFileName(str6);
                        if (UnLog.D) {
                            UnLog.d("custom_theme", "compare filePath " + str7);
                        }
                        File file2 = new File(str7);
                        if (!file2.exists()) {
                            hashMap3.put(str6, dataCategory2.get(str6));
                            hashMap = dataCategory2;
                        } else {
                            for (ImageInfoEntity imageInfoEntity : dataCategory2.get(str6)) {
                                imageInfoEntity.localPath = str7;
                                parseImageData2.put(imageInfoEntity.imageId, imageInfoEntity);
                                dataCategory2 = dataCategory2;
                            }
                            hashMap = dataCategory2;
                            try {
                                readLocalFile2.remove(file2);
                            } catch (Exception unused2) {
                            }
                        }
                        dataCategory2 = hashMap;
                    }
                    this.deleteImageFile.addAll(readLocalFile2);
                }
                if (hashMap3.isEmpty()) {
                    if (checkChange(parseImageData2)) {
                        if (TextUtils.equals(str5, getCurThemeId())) {
                            themeChange(str5, parseImageData2);
                            return;
                        } else {
                            sendThemeInfo(str5, parseImageData2);
                            return;
                        }
                    }
                    CustomDBController.deleteNavi();
                    List<NavigationInfo> list = this.navigationInfos;
                    if (list != null && list.size() > 0) {
                        CustomDBController.insertNavis(this.navigationInfos);
                    }
                ...
                prepareDownload(str5, hashMap3, parseImageData2, z);
                return;
            }
            notifyDownloadThemeResult(str5, false);
            return;
