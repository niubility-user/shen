...
public class JDAddressSelectView extends RelativeLayout implements JDSlideBar.ISlideBarTouchChangeLisener, OnViewThemeConfig<JDAddressSelectView> {
    ...
    */
    private boolean getHotCityHeaderView() {
        String str;
        final List<HotAreaBean> parseArray;
        final List<HotAreaBean> parseArray2;
        int i2;
        int i3;
        int i4;
        int i5;
        ...
        if (this.sumTabSwitch) {
            try {
                ...
                try {
                    ...
                    if (!TextUtils.isEmpty(str)) {
                        parseArray = JDJSON.parseArray(str, HotAreaBean.class);
                        parseArray2 = JDJSON.parseArray(str2, HotAreaBean.class);
                        if (UnLog.D) {
                        }
                        if (parseArray.size() >= 1) {
                            ...
                            gridView.setOnItemClickListener(new AdapterView.OnItemClickListener() { // from class: com.jingdong.common.ui.JDAddressSelectView.13
                                ...
                                public void onItemClick(AdapterView<?> adapterView, View view, int i6, long j2) {
                                    ...
                                    int i7 = jDAddressSelectView.mSumCurrent;
                                    if (i7 == 101) {
                                        if (i6 < 0 || i6 >= parseArray.size()) {
                                            return;
                                        }
                                        AddressGlobal addressGlobal = new AddressGlobal();
                                        addressGlobal.setIdProvince(JDAddressSelectView.this.getCommonId(((HotAreaBean) parseArray.get(i6)).getProvinceId()));
                                        addressGlobal.setProvinceName(((HotAreaBean) parseArray.get(i6)).getProvinceName());
                                        if (!TextUtils.isEmpty(((HotAreaBean) parseArray.get(i6)).getCityId())) {
                                            addressGlobal.setIdCity(JDAddressSelectView.this.getCommonId(((HotAreaBean) parseArray.get(i6)).getCityId()));
                                            addressGlobal.setCityName(((HotAreaBean) parseArray.get(i6)).getCityName());
                                        } else {
                                    ...
                                    } else if (i7 != 102 || i6 < 0 || i6 >= parseArray2.size() || JDAddressSelectView.this.cityList == null) {
                                    } else {
                                        int defaultPosition = JDAddressSelectView.this.getDefaultPosition(((HotAreaBean) parseArray2.get(i6)).getCityId(), JDAddressSelectView.this.cityList);
                                        if (UnLog.D) {
                                            UnLog.d(JDAddressSelectView.TAG, "hot_seas_click = " + i6);
                                        }
                                        if (defaultPosition != -1) {
                                            JDAddressSelectView jDAddressSelectView2 = JDAddressSelectView.this;
                                            jDAddressSelectView2.mCurrent = 3;
                                            jDAddressSelectView2.next(defaultPosition);
                                        }
                                    }
                                }
                            });
            ...
            if (UnLog.D) {
                String str3 = TAG;
                UnLog.d(str3, "inlandSrt = " + str);
                UnLog.d(str3, "overSeasStr = " + str2);
            }
            if (!TextUtils.isEmpty(str) && !TextUtils.isEmpty(str2)) {
                ...
                if (UnLog.D) {
                    String str4 = TAG;
                    UnLog.d(str4, "inlandListSize = " + parseArray.size());
                    UnLog.d(str4, "overSeasListSize = " + parseArray2.size());
                }
                if (parseArray.size() >= 1 && parseArray2.size() >= 1) {
                    this.mHotCityHeaderView = LayoutInflater.from(getContext()).inflate(R.layout.jd_address_hot_city, (ViewGroup) null);
                    this.gridAdapter = new JDAddressGridAdapter(getContext(), parseArray, isDarkMode());
                    GridView gridView2 = (GridView) this.mHotCityHeaderView.findViewById(R.id.hot_area_grid_new);
                    TextView textView2 = (TextView) this.mHotCityHeaderView.findViewById(R.id.hot_text_view1);
                    if (!isDarkMode()) {
                        this.mHotCityHeaderView.setBackgroundColor(getResources().getColor(R.color.un_bg_level_2_dark));
                        textView2.setTextColor(getResources().getColor(R.color.un_content_level_2_dark));
                    } else {
                        this.mHotCityHeaderView.setBackgroundColor(getResources().getColor(R.color.un_bg_level_2));
                        textView2.setTextColor(getResources().getColor(R.color.un_content_level_2));
                    }
                    i2 = this.mSumCurrent;
                    if (i2 != 101) {
                        textView2.setText(R.string.un_address_hot_city);
                    } else if (i2 == 102) {
                        textView2.setText(R.string.un_address_hot_area);
                    }
                    if (this.mType != 1) {
                    ...
                    gridView2.setOnItemClickListener(new AdapterView.OnItemClickListener() { // from class: com.jingdong.common.ui.JDAddressSelectView.13
                        @Override // android.widget.AdapterView.OnItemClickListener
                        public void onItemClick(AdapterView<?> adapterView, View view, int i6, long j2) {
                            if (JDAddressSelectView.this.mIsDestroy) {
                                return;
                    ...
                    this.mHotCityHeaderHeight = (i5 * 42) + 28;
                    if (UnLog.D) {
                        String str5 = TAG;
                        UnLog.d(str5, "hot_city_line = " + i5);
                        UnLog.d(str5, "hot_city_measuredHeight = " + this.mHotCityHeaderHeight);
                    }
