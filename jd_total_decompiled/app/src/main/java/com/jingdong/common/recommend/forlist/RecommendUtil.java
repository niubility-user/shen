...
public class RecommendUtil {
    ...
    */
    public void onBindNewRecommendViewHolder(RecyclerView.ViewHolder viewHolder, int i2, BaseActivity baseActivity, List list) {
        if (viewHolder instanceof RecommendViewHolder) {
            RecommendViewHolder recommendViewHolder = (RecommendViewHolder) viewHolder;
            recommendViewHolder.setExpoDataShop(this.expoDataShop);
            recommendViewHolder.setRealExpoDataStore(this.realExpoDataStore, this.realExpoDataAggreation);
            ...
            if (!this.isAdAllPositionExpo && i2 > 6) {
                ...
                if (list != null) {
                    try {
                        if (!list.isEmpty()) {
                            ((RecommendViewHolder) viewHolder).bindNewRecommendViewHolder(list, this.mRecommendItemList, i2, this.expoDataStore, this.from, this.manager.tabId, this.jdDisplayImageOptions, this.expoDataAggreation, this.expoDataDetails, this.expoDataQuesitionNair);
                        }
                    } catch (Exception e2) {
                        e2.printStackTrace();
                        return;
