    ...
    public class PrivateOnTouchListener implements View.OnTouchListener {
        ...
        */
        public boolean onTouch(View view, MotionEvent motionEvent) {
            SimpleTouchImageView.this.mScaleDetector.onTouchEvent(motionEvent);
            SimpleTouchImageView.this.mGestureDetector.onTouchEvent(motionEvent);
            PointF pointF = new PointF(motionEvent.getX(), motionEvent.getY());
            State state = SimpleTouchImageView.this.state;
            State state2 = State.NONE;
            if (state == state2 || SimpleTouchImageView.this.state == State.DRAG || SimpleTouchImageView.this.state == State.FLING) {
                int action = motionEvent.getAction();
                if (action != 0) {
                    if (action != 1) {
                        if (action == 2) {
                            if (SimpleTouchImageView.this.state == State.DRAG) {
                                float f2 = pointF.x;
                                PointF pointF2 = this.last;
                                float f3 = f2 - pointF2.x;
                                float f4 = pointF.y - pointF2.y;
                                SimpleTouchImageView.this.matrix.postTranslate(SimpleTouchImageView.this.getFixDragTrans(f3, r2.viewWidth, SimpleTouchImageView.this.getImageWidth()), SimpleTouchImageView.this.getFixDragTrans(f4, r2.viewHeight, SimpleTouchImageView.this.getImageHeight()));
                                SimpleTouchImageView.this.fixTrans();
                                this.last.set(pointF.x, pointF.y);
                            }
                        }
                    }
                    SimpleTouchImageView.this.setState(state2);
                } else {
                    this.last.set(pointF);
                    if (SimpleTouchImageView.this.fling != null) {
                        SimpleTouchImageView.this.fling.cancelFling();
                    }
                    SimpleTouchImageView.this.setState(State.DRAG);
                }
            }
            SimpleTouchImageView simpleTouchImageView = SimpleTouchImageView.this;
            simpleTouchImageView.setImageMatrix(simpleTouchImageView.matrix);
            if (SimpleTouchImageView.this.userTouchListener != null) {
                SimpleTouchImageView.this.userTouchListener.onTouch(view, motionEvent);
            }
            if (SimpleTouchImageView.this.touchImageViewListener != null) {
                SimpleTouchImageView.this.touchImageViewListener.onMove();
            }
