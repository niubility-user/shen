    ...
    */
    public void notifyMessageToNative(String str) {
        ...
        try {
            ...
            JsInputEntity jsInputEntity = (JsInputEntity) JDJSON.parseObject(str, JsInputEntity.class);
            String str2 = jsInputEntity.businessType;
            String str3 = jsInputEntity.callBackName;
            char c2 = '\uffff';
            if (str2.hashCode() == 548553333 && str2.equals("unionFingerPrint")) {
                c2 = 0;
            }
            ...
            if (!TextUtils.isEmpty(unionFingerPrint)) {
                jSONObject = new JSONObject(unionFingerPrint);
                jSONObject.put("jdkey", JMA.getJDKey(this.webUiBinder.getBaseActivity().getApplicationContext()));
                if (OKLog.D) {
                    OKLog.d(TAG, "[sgact]webview get jdkey+" + jSONObject);
                }
            }
            WebUtils.sendJSONStr2M(this.webUiBinder, str3, WebUtils.stringfyJSonData("0", jSONObject, ""));
        } catch (Exception e2) {
            e2.printStackTrace();
        }
    }
