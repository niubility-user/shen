...
public class PriceHelper {
    ...
    */
    public static void handleNewPrice(@NonNull Context context, OriginPriceBean originPriceBean) {
        ...
        int i2 = 0;
        while (true) {
            if (i2 >= originPriceBean.renewVersionPrice.size()) {
                str = null;
                break;
            }
            PriceInfoBean priceInfoBean2 = originPriceBean.renewVersionPrice.get(i2);
            if (priceInfoBean2 != null && (str2 = priceInfoBean2.type) != null) {
                if (TextUtils.equals("price", str2.trim())) {
                    if (showCommonPrice(context, priceInfoBean2, originPriceBean, i2) > -1) {
                        arrayList.add(Integer.valueOf(i2));
                    }
                } else {
                    if (TextUtils.equals("icon", priceInfoBean2.type.trim())) {
                        String str3 = TextUtils.isEmpty(priceInfoBean2.iconCodeDark) ? priceInfoBean2.iconCode : priceInfoBean2.iconCodeDark;
                        if (DeepDarkChangeManager.getInstance().getUIMode() != 1) {
                            str3 = priceInfoBean2.iconCode;
                        }
                        String str4 = str3;
                        if (!TextUtils.isEmpty(str4)) {
                            priceInfoBean = priceInfoBean2;
                            if (showCommonPriceByLegao(context, priceInfoBean2.leftMargin, priceInfoBean2.uniqueId, priceInfoBean2.iconText, str4, linearLayout, originPriceBean, priceInfoBean2.describe, i2) > -1) {
                                arrayList.add(Integer.valueOf(i2));
                            }
                        }
                    } else {
                        priceInfoBean = priceInfoBean2;
                        if (TextUtils.equals("text", priceInfoBean.type.trim()) && showPriceTextLabel(context, priceInfoBean, originPriceBean, i2) > -1) {
                            arrayList.add(Integer.valueOf(i2));
                        }
                    }
                    if (originPriceBean.priceMaxWidth > 0) {
                        ...
                        for (int childCount = linearLayout.getChildCount() - 1; childCount >= 0; childCount--) {
                            ...
                            if (childAt2 != null && i2 != 0 && (childAt2.getTag() instanceof String) && !TextUtils.isEmpty(priceInfoBean.uniqueId) && TextUtils.equals((String) childAt2.getTag(), priceInfoBean.uniqueId.trim())) {
