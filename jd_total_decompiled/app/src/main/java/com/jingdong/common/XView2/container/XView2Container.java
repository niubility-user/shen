    ...
    */
    public boolean dispatchTouchEvent(MotionEvent motionEvent) {
        ...
        if (action != 0) {
            if (action != 1) {
                if (action == 2) {
                    if (!"1".equals(this.mLayersEntity.layout.fill)) {
                        if (!this.startMove) {
                        ...
                        if (this.startMove && XView2Utils.isCanDrag(this.mLayersEntity.dragMode)) {
                            ...
                            if ("3".equals(this.mLayersEntity.dragMode)) {
                                ...
                                int bottom = viewGroup != null ? viewGroup.getBottom() - this.mRootView.getPaddingBottom() : 0;
                                if (y2 >= 0.0f && y2 < bottom - getHeight()) {
                                    setX(x2);
                                    setY(y2);
                                }
                            }
            ...
            if (this.startMove && XView2Utils.isCanDrag(this.mLayersEntity.dragMode)) {
                ...
                XViewConfigEntity.LayersEntity layersEntity = this.mLayersEntity;
                if (layersEntity != null && layersEntity.renderData != null && this.mContentContainer != null) {
                    Rect rect2 = new Rect();
                    if (this.mCloseBtn != null) {
                        rect = new Rect();
                        this.mCloseBtn.getGlobalVisibleRect(rect);
                    } else {
                        rect = null;
                    }
                    this.mContentContainer.getGlobalVisibleRect(rect2);
                    float rawX = this.mLayersEntity.renderType == 6 ? motionEvent.getRawX() : motionEvent.getX();
                    float rawY = this.mLayersEntity.renderType == 6 ? motionEvent.getRawY() : motionEvent.getY();
                    if (!XView2Utils.isInRect(rawX, rawY, rect2, rect)) {
                        if (this.mIContainerControlListener != null) {
                            if (this.mBaseLayerDelegate != null) {
                                JDJSONObject jDJSONObject = new JDJSONObject();
                                jDJSONObject.put("name", (Object) "xview");
                                BaseLayerDelegate baseLayerDelegate = this.mBaseLayerDelegate;
                                XViewConfigEntity.LayersEntity layersEntity2 = this.mLayersEntity;
                                baseLayerDelegate.onClickCallBack(layersEntity2.layerId, layersEntity2.logicKey, 4, jDJSONObject.toJSONString());
                            }
                            if (XView2Utils.isConvertBool(this.mLayersEntity.renderData.blankCls)) {
                                this.mIContainerControlListener.closeCurrentLayer();
                            }
                        }
                    } else if (this.mLayersEntity.renderType == 6) {
                        dealTouchArea(this.mContentContainer.getChildAt(0), rawX, rawY);
                    } else {
                        ...
                        while (true) {
                            ...
                            if (childAt != null) {
                                ...
                                boolean isInRect = XView2Utils.isInRect(rawX, rawY, rect3, null);
                                this.mIsInRectArea = isInRect;
                                if (isInRect) {
                                    XViewLogPrint.JDXLog.e(XView2Constants.TAG, "\u5728\u70b9\u51fb\u533a\u57df\u5185");
                                    break;
