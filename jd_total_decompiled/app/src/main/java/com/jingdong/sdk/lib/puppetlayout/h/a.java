...
public abstract class a {
    ...
    /* JADX WARN: Removed duplicated region for block: B:155:0x0156 A[ADDED_TO_REGION] */
    /* JADX WARN: Removed duplicated region for block: B:158:0x0173  */
    /* JADX WARN: Removed duplicated region for block: B:160:0x007c A[EXC_TOP_SPLITTER, SYNTHETIC] */
    /* JADX WARN: Removed duplicated region for block: B:166:0x00f2 A[EXC_TOP_SPLITTER, SYNTHETIC] */
    /*
    ...
    */
    private boolean i() {
        ...
        if (this.d.size() == 1 && !TextUtils.isEmpty(this.d.get(ViewProps.BACKGROUND_COLOR))) {
            try {
                this.a.setBackgroundColor(Color.parseColor(this.d.get(ViewProps.BACKGROUND_COLOR)));
            } catch (Exception e2) {
                e2.printStackTrace();
            }
            return true;
        }
        boolean equals = "1".equals(this.d.get("roundAsCircle"));
        String str = this.d.get(ViewProps.BACKGROUND_COLOR);
        if (TextUtils.isEmpty(str)) {
            str = this.d.get(DYConstants.DY_BG_COLOR);
        }
        if (str != null) {
            try {
            ...
            if (this.d.get("highlightBackgroundColor") != null) {
                try {
                    parseColor2 = Color.parseColor(this.d.get("highlightBackgroundColor"));
                    z2 = true;
                } catch (Exception e4) {
                    e4.printStackTrace();
                }
                dpValue = LayoutUtils.getDpValue(this.d.get("roundCornerRadius"), -1.0f);
                if (dpValue != -1.0f) {
                    f4 = dpValue;
                    f3 = f4;
                    f2 = f3;
                    ...
                    float dpValue3 = LayoutUtils.getDpValue(this.d.get("roundCornerTopLeft"), -1.0f);
                    float dpValue4 = LayoutUtils.getDpValue(this.d.get("roundCornerTopRight"), -1.0f);
                    float dpValue5 = LayoutUtils.getDpValue(this.d.get("roundCornerBottomLeft"), -1.0f);
                    dpValue2 = LayoutUtils.getDpValue(this.d.get("roundCornerBottomRight"), -1.0f);
                    f2 = dpValue5;
                    f3 = dpValue4;
                    f4 = dpValue3;
                }
                if (this.d.get("borderColor") != null) {
                    try {
                        parseColor3 = Color.parseColor(this.d.get("borderColor"));
                        z3 = true;
                    } catch (Exception e5) {
                        e5.printStackTrace();
                    }
                    float dpValue6 = LayoutUtils.getDpValue(this.d.get("borderWidth"), -1.0f);
                    float[] fArr = {Math.max(0.0f, f4), Math.max(0.0f, f4), Math.max(0.0f, f3), Math.max(0.0f, f3), Math.max(0.0f, dpValue2), Math.max(0.0f, dpValue2), Math.max(0.0f, f2), Math.max(0.0f, f2)};
                    if (!z && z2) {
                        int i2 = (int) dpValue6;
                        DrawableUtils.setCompactBg(this.a, DrawableUtils.createStateBg(parseColor, true, i2, parseColor3, z3, fArr, parseColor2, true, i2, parseColor3, z3, fArr));
                    } else if (z) {
                        DrawableUtils.setCompactBg(this.a, DrawableUtils.createNormalBg(parseColor, true, (int) dpValue6, parseColor3, z3, fArr, equals));
                    }
                    return true;
                }
                parseColor3 = 0;
                z3 = false;
                float dpValue62 = LayoutUtils.getDpValue(this.d.get("borderWidth"), -1.0f);
                float[] fArr2 = {Math.max(0.0f, f4), Math.max(0.0f, f4), Math.max(0.0f, f3), Math.max(0.0f, f3), Math.max(0.0f, dpValue2), Math.max(0.0f, dpValue2), Math.max(0.0f, f2), Math.max(0.0f, f2)};
                if (!z) {
                }
                if (z) {
                }
            ...
            parseColor3 = 0;
            z3 = false;
            float dpValue622 = LayoutUtils.getDpValue(this.d.get("borderWidth"), -1.0f);
            float[] fArr22 = {Math.max(0.0f, f4), Math.max(0.0f, f4), Math.max(0.0f, f3), Math.max(0.0f, f3), Math.max(0.0f, dpValue2), Math.max(0.0f, dpValue2), Math.max(0.0f, f2), Math.max(0.0f, f2)};
            if (!z) {
            }
            if (z) {
            }
        ...
        parseColor3 = 0;
        z3 = false;
        float dpValue6222 = LayoutUtils.getDpValue(this.d.get("borderWidth"), -1.0f);
        float[] fArr222 = {Math.max(0.0f, f4), Math.max(0.0f, f4), Math.max(0.0f, f3), Math.max(0.0f, f3), Math.max(0.0f, dpValue2), Math.max(0.0f, dpValue2), Math.max(0.0f, f2), Math.max(0.0f, f2)};
        if (!z) {
        }
        if (z) {
        }
