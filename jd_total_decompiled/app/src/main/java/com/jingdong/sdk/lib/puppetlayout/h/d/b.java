...
public class b {
    ...
    */
    public static void b(String str, String str2, SparseArray<Float> sparseArray, SparseArray<String> sparseArray2) {
        boolean z;
        float f2;
        boolean z2;
        boolean z3 = true;
        if (b.containsKey(str) && (str2.endsWith("%") || str2.equals("auto"))) {
            try {
                if (str2.endsWith("%")) {
                    Float.parseFloat(str2.substring(0, str2.length() - 1));
                }
                z2 = true;
            ...
            if (z2) {
                sparseArray2.put(b.get(str).intValue(), str2);
                z = true;
                if (z && a.containsKey(str)) {
                    float f3 = 0.0f;
                    try {
                        f2 = LayoutUtils.getValue(str, str2);
                        if (f2 == -1.0f) {
                            try {
                                if (LayoutUtils.canUnitDp(str)) {
                                    if (!"0".equals(str2)) {
                                        f3 = LayoutUtils.getDpValue(str2);
                                    }
                                } else {
                    ...
                    sparseArray.put(a.get(str).intValue(), Float.valueOf(f2));
                    return;
                }
            }
        }
