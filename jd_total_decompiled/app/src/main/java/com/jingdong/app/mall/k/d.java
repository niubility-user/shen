        ...
        */
        public void onBackToForeground() {
            ...
            if (JDPrivacyHelper.isAcceptPrivacy(JdSdk.getInstance().getApplicationContext())) {
                com.jingdong.app.mall.home.o.a.d.l(this.a);
                a();
                ...
                if (this.a > 0 && BaseFrameUtil.getInstance().getCurrentMyActivity() != null) {
                    JDMtaUtils.onClickWithPageId(BaseFrameUtil.getInstance().getCurrentMyActivity().getThisActivity(), "App_Wakeup", g.class.getName(), (System.currentTimeMillis() - this.a) + "", "");
                    JDJSONObject openJsonParam = OpenLinkTimeManager.getInstance().getOpenJsonParam(OpenLinkTimeManager.getInstance().getLatestSequence());
                    if (openJsonParam == null) {
                        try {
                            JDJSONObject jDJSONObject = new JDJSONObject();
                            try {
                                jDJSONObject.put("coldOrHot", (Object) "2");
                                jDJSONObject.put("sourceApp", (Object) "");
                                jDJSONObject.put("openAppUrl", (Object) "");
                                openJsonParam = jDJSONObject;
                            } catch (Exception e2) {
                                e = e2;
                                openJsonParam = jDJSONObject;
                                e.printStackTrace();
                                JDMtaUtils.sendClickDataWithExt(BaseFrameUtil.getInstance().getCurrentMyActivity().getThisActivity(), "App_Wakeup_New", (System.currentTimeMillis() - this.a) + "", "", "", g.class.getName(), "", "", openJsonParam != null ? openJsonParam.toString() : "", null);
                                ...
                                if (XView2Manager.mIsXViewEnable) {
                                    XView2Manager.getInstance().requestXViewData();
                                }
                                if (CaasKitHelper.getInstance().isCaasKitInit()) {
                                    CaasKitHelper.getInstance().resumeShare();
                                }
                                JDSkinSDK.getInstance().getResData(AppStateType.APP_TO_BEFORE);
                                DynamicPrepareFetcher.prepareFetch(DynamicPrepareFetcher.KEY_PREPARE_MODEL_FOREGROUND);
                            }
                        } catch (Exception e3) {
                            e = e3;
                        }
