...
public class LibPdBottomBtnPresenter {
    ...
    */
    public void doYuyue(final LibPdStyleInfoViewYuYueListener libPdStyleInfoViewYuYueListener) {
        WareBusinessData wareBusinessData;
        boolean z;
        ProductDetailEntity productDetailEntity = this.mProduct;
        if (productDetailEntity == null || (wareBusinessData = productDetailEntity.mWareBusinessData) == null || wareBusinessData.yuyueInfo == null) {
            return;
        }
        if (productDetailEntity.isPlusMakeTime()) {
            ...
            if (wareBusinessPlusForBuyMt.makeMoreTimeFlag && ((wareBusinessPlusForBuyMt.drawMoreTimeFlag || wareBusinessPlusForBuyMt.appointMoreTimeFlag) && wareBusinessPlusForBuyMt.userFlag && wareBusinessData2.yuyueInfo.pinSkuYuYue)) {
                ...
                if (!z) {
                    Activity activity = this.mActivity;
                    PDUtils.showToastCenterNormal(activity, activity.getString(R.string.libpdstyleinfoview_pd_yuyue_success));
                    return;
                }
                if (this.mProduct.getStartTime() > 0) {
                    String str = "{\"des\":\"productDetail\",\"params\":{\"skuId\":\"" + this.mProduct.skuId + "\"}}";
                    JDReminderNewUtils.setReminder("RESERVE", this.mActivity.getString(R.string.libpdstyleinfoview_yuyue), "detailReserve_" + this.mProduct.skuId, this.mProduct.getProductDetailName(), "", this.mProduct.getStartTime() * 1000, 0L, str, "", "");
                }
                if (this.mProduct.mWareBusinessData.yuyueInfo.mad) {
                    if (LoginUserBase.hasLogin()) {
                        PDIsAppointProtocol pDIsAppointProtocol = new PDIsAppointProtocol(this.mProduct.mManageKey);
                        pDIsAppointProtocol.setViewYuYueListener(libPdStyleInfoViewYuYueListener);
                        ProductDetailEntity productDetailEntity2 = this.mProduct;
                        pDIsAppointProtocol.setInputParams(productDetailEntity2.skuId, "1", productDetailEntity2.getYuyueShowCode());
                        Activity activity2 = this.mActivity;
                        if (activity2 instanceof BaseActivity) {
                            ((BaseActivity) activity2).addHttpGroupWithNPSSetting(pDIsAppointProtocol.request());
                            return;
                        }
                        return;
                    }
                    LoginUserHelper.getInstance().executeLoginRunnable(this.mIMyActivity, new Runnable() { // from class: com.jingdong.app.mall.bundle.styleinfoview.presenter.LibPdBottomBtnPresenter.3
                        @Override // java.lang.Runnable
                        public void run() {
                            LibPdStyleInfoViewYuYueListener libPdStyleInfoViewYuYueListener2 = libPdStyleInfoViewYuYueListener;
                            if (libPdStyleInfoViewYuYueListener2 != null) {
                                libPdStyleInfoViewYuYueListener2.onLogin();
                            }
                        }
                    });
                    return;
                }
                LoginUserHelper.getInstance().executeLoginRunnable(this.mIMyActivity, new Runnable() { // from class: com.jingdong.app.mall.bundle.styleinfoview.presenter.LibPdBottomBtnPresenter.4
                    @Override // java.lang.Runnable
                    public void run() {
                        ...
                        boolean z2 = false;
                        if (LibPdBottomBtnPresenter.this.mProduct.isPlusSwaying()) {
                            Bundle bundle = new Bundle();
                            if (LibPdBottomBtnPresenter.this.mProduct.mWareBusinessData != null && LibPdBottomBtnPresenter.this.mProduct.mWareBusinessData.makeMoreTime != null && LibPdBottomBtnPresenter.this.mProduct.mWareBusinessData.makeMoreTime.userFlag) {
                                z2 = true;
                            }
                            bundle.putString(AddressConstant.ADDRESS_LIST_BUSSINESS_TYPE_KEY, AddressConstant.ADDRESS_LIST_BUSSINESS_PRODUCTDETAIL_TYPE);
                            bundle.putString(AddressConstant.ADDRESS_LIST_SOURCE_MAP_KEY, DeepLinkAddressHelper.getSourceMapJsonString("805fae4944f244f193ba7b937b99db12", z2));
                            DeepLinkAddressHelper.startNewEasyAddressListDynamic(LibPdBottomBtnPresenter.this.mActivity, bundle, true, 38);
                            return;
                        }
                        PDIsAppointProtocol pDIsAppointProtocol2 = new PDIsAppointProtocol(LibPdBottomBtnPresenter.this.mProduct.mManageKey);
                        pDIsAppointProtocol2.setViewYuYueListener(libPdStyleInfoViewYuYueListener);
                        pDIsAppointProtocol2.setInputParams(LibPdBottomBtnPresenter.this.mProduct.skuId, "1", LibPdBottomBtnPresenter.this.mProduct.getYuyueShowCode());
                        if (LibPdBottomBtnPresenter.this.mActivity instanceof BaseActivity) {
                            ((BaseActivity) LibPdBottomBtnPresenter.this.mActivity).addHttpGroupWithNPSSetting(pDIsAppointProtocol2.request());
                        }
                    }
                });
                return;
