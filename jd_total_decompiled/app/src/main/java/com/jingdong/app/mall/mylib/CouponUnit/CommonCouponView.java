...
public class CommonCouponView extends FrameLayout {
    ...
    */
    protected void handleFaceType(CouponEntity couponEntity) {
        if (couponEntity == null || TextUtils.isEmpty(couponEntity.faceType)) {
            return;
        }
        if (couponEntity.getTypeCoupon() == 401) {
            AppCompatTextView appCompatTextView = this.couponValue;
            Context context = this.context;
            int i2 = R.color.coupon_C25900;
            appCompatTextView.setTextColor(ContextCompat.getColor(context, i2));
            this.startUsingConditionOrExceptionCondition.setTextColor(ContextCompat.getColor(this.context, i2));
        } else {
            AppCompatTextView appCompatTextView2 = this.couponValue;
            Context context2 = this.context;
            int i3 = R.color.coupon_FFFFFF;
            appCompatTextView2.setTextColor(ContextCompat.getColor(context2, i3));
            this.startUsingConditionOrExceptionCondition.setTextColor(ContextCompat.getColor(this.context, i3));
        }
        String str = couponEntity.withAvailable;
        char c2 = 0;
        if (!TextUtils.isEmpty(str)) {
        ...
        this.couponValue.getPaint();
        FontsUtils.changeTextFont(this.couponValue, 4097);
        String str2 = couponEntity.faceType;
        str2.hashCode();
        ...
        switch (c2) {
            case 0:
                try {
                    TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(this.couponValue, 2, 44, 1, 1);
                    this.couponValue.setText(CouponUtil.TransNativeNormalValue(getContext(), couponEntity.getFaceValue()));
                    break;
                } catch (Exception unused) {
                    this.couponValue.setText(couponEntity.getFaceValue());
                    break;
                }
            case 1:
                TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(this.couponValue, 2, 15, 1, 1);
                this.couponValue.setText(CouponUtil.TransDiscountString(couponEntity.getFaceValue(), 2.9333334f, 1.5f));
                break;
            case 2:
                TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(this.couponValue, 2, 12, 1, 1);
                this.couponValue.setText(CouponUtil.TransDiscountString(couponEntity.getFaceValue(), 2.9166667f, 1.5f));
                break;
            case 3:
                TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(this.couponValue, 2, 30, 1, 1);
                setValueBackgroundDrawable(R.drawable.value_background_finance);
                this.couponValue.setText(couponEntity.getFaceValue());
                break;
            case 4:
                TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(this.couponValue, 2, 15, 1, 1);
                this.couponValue.setText(CouponUtil.TransFinaceString(couponEntity.getFaceValue(), 2.3333333f));
                break;
            case 5:
                TextViewCompat.setAutoSizeTextTypeUniformWithConfiguration(this.couponValue, 2, 15, 1, 1);
                this.couponValue.setText(CouponUtil.TransFinaceString(couponEntity.getFaceValue(), 2.9333334f));
                break;
        }
        this.couponValue.invalidate();
    }
