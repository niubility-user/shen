...
public class VideoParameterSet {
    ...
    */
    private void hrd_parameters(boolean z, int i2, CAVLCReader cAVLCReader) throws IOException {
        boolean z2;
        boolean z3;
        boolean z4;
        if (z) {
            ...
            if (z2 || z3) {
                ...
                for (int i3 = 0; i3 <= i2; i3++) {
                    zArr[i3] = cAVLCReader.readBool("fixed_pic_rate_general_flag[" + i3 + "]");
                    if (!zArr[i3]) {
                        zArr2[i3] = cAVLCReader.readBool("fixed_pic_rate_within_cvs_flag[" + i3 + "]");
                    }
                    if (zArr2[i3]) {
                        iArr2[i3] = cAVLCReader.readUE("elemental_duration_in_tc_minus1[" + i3 + "]");
                    } else {
                        zArr3[i3] = cAVLCReader.readBool("low_delay_hrd_flag[" + i3 + "]");
                    }
                    if (!zArr3[i3]) {
                        iArr[i3] = cAVLCReader.readUE("cpb_cnt_minus1[" + i3 + "]");
                    }
                    if (z2) {
                        sub_layer_hrd_parameters(i3, iArr[i3], z4, cAVLCReader);
                    }
                    if (z3) {
                        sub_layer_hrd_parameters(i3, iArr[i3], z4, cAVLCReader);
                    }
