...
@Immutable(containerOf = {AuraConstants.MESSAGE_COUPON_TYPE_NEW, "E"})
@Beta
...
public final class ImmutableNetwork<N, E> extends ConfigurableNetwork<N, E> {
    ...
    private static <N, E> NetworkConnections<N, E> connectionsOf(Network<N, E> network, N n2) {
        if (network.isDirected()) {
            Map asMap = Maps.asMap(network.inEdges(n2), sourceNodeFn(network));
            Map asMap2 = Maps.asMap(network.outEdges(n2), targetNodeFn(network));
        ...
        Map asMap3 = Maps.asMap(network.incidentEdges(n2), adjacentNodeFn(network, n2));
        if (network.allowsParallelEdges()) {
