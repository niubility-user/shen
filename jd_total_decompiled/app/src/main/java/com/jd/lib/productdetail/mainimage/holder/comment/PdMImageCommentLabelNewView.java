...
public class PdMImageCommentLabelNewView extends ConstraintLayout {

    /* renamed from: g  reason: collision with root package name */
    public PdMCommentLabelFlowLayout f4717g;

    public PdMImageCommentLabelNewView(Context context, AttributeSet attributeSet) {
        super(context, attributeSet);
    }

    public void a(List<PdCommentInfo.CommentTag> list) {
        if (list != null) {
            this.f4717g.removeAllViews();
            for (PdCommentInfo.CommentTag commentTag : list) {
                if (!TextUtils.isEmpty(commentTag.name)) {
                    ...
                    this.f4717g.addView(pdAutoChangeTextSize);
                }
            }
        }
    }
    ...
    public void onFinishInflate() {
        ...
        PdMCommentLabelFlowLayout pdMCommentLabelFlowLayout = (PdMCommentLabelFlowLayout) findViewById(R.id.lib_pd_holder_topimage_item_comment_label_container_new);
        this.f4717g = pdMCommentLabelFlowLayout;
        if (pdMCommentLabelFlowLayout != null) {
            int dip2px = PDUtils.dip2px(12.0f);
            PdMCommentLabelFlowLayout pdMCommentLabelFlowLayout2 = this.f4717g;
