        ...
        */
        public CloseableImage decode(EncodedImage encodedImage, int i2, QualityInfo qualityInfo, ImageDecodeOptions imageDecodeOptions) {
            ...
            if (FrescoSystrace.isTracing()) {
                FrescoSystrace.beginSection("AvifFormatDecoder#decode");
            }
            if (encodedImage.getImageFormat() != AvifDecoderUtil.AVIF_FORMAT) {
                return null;
            }
            try {
                try {
                    ...
                    if (!AvifDecoder.getInfo(allocateDirect, allocateDirect.remaining(), info)) {
                        FLog.d(AvifDecoderUtil.TAG, "getInfo : failed");
                        throw new DecodeException("avif decode error", encodedImage);
                    }
                    FLog.d(AvifDecoderUtil.TAG, "\u89e3\u7801\uff1a\u56fe\u7247\u4fe1\u606f :  width: " + info.width + " height: " + info.height + " depth: " + info.depth);
                    Bitmap createBitmap = Bitmap.createBitmap(info.width, info.height, Bitmap.Config.ARGB_8888);
                    if (!AvifDecoder.decode(allocateDirect, allocateDirect.remaining(), createBitmap)) {
                        FLog.d(AvifDecoderUtil.TAG, "AvifDecoder.decode : failed");
                        throw new DecodeException("avif decode error", encodedImage);
                    }
                    if (!AvifDecoderUtil.isAVIFBitmapConvertEnable()) {
                        if (AvifDecoderUtil.isAVIFBitmapConvertOnlyAlphaEnable()) {
                            byte[] bArr = new byte[100];
                            if (allocateDirect.array().length > 250) {
                                System.arraycopy(allocateDirect.array(), 150, bArr, 0, 100);
                            }
                            if (new String(bArr).contains(AvifDecoderUtil.AV_01_ALPHA)) {
                                if (BaseInfo.getAndroidSDKVersion() <= 30) {
                                    throw new DecodeException("avif decode alpha image on Android" + BaseInfo.getAndroidVersion(), encodedImage);
                                }
                            }
                        }
                        CloseableStaticBitmap closeableStaticBitmap = new CloseableStaticBitmap(createBitmap, SimpleBitmapReleaser.getInstance(), ImmutableQualityInfo.FULL_QUALITY, encodedImage.getRotationAngle(), encodedImage.getExifOrientation());
                        FLog.d(AvifDecoderUtil.TAG, "\u89e3\u7801: \u4f4d\u56fe\u5927\u5c0f(\u5b57\u8282) : " + createBitmap.getByteCount());
                        FLog.d(AvifDecoderUtil.TAG, "\u89e3\u7801: \u8017\u65f6\uff08ms\uff09 : " + (System.currentTimeMillis() - currentTimeMillis));
                        if (FrescoSystrace.isTracing()) {
                            FrescoSystrace.endSection();
                        }
                        return closeableStaticBitmap;
                    }
                    createBitmap = AvifDecoderUtil.convertBitmap(createBitmap);
                    CloseableStaticBitmap closeableStaticBitmap2 = new CloseableStaticBitmap(createBitmap, SimpleBitmapReleaser.getInstance(), ImmutableQualityInfo.FULL_QUALITY, encodedImage.getRotationAngle(), encodedImage.getExifOrientation());
                    FLog.d(AvifDecoderUtil.TAG, "\u89e3\u7801: \u4f4d\u56fe\u5927\u5c0f(\u5b57\u8282) : " + createBitmap.getByteCount());
                    FLog.d(AvifDecoderUtil.TAG, "\u89e3\u7801: \u8017\u65f6\uff08ms\uff09 : " + (System.currentTimeMillis() - currentTimeMillis));
                    if (FrescoSystrace.isTracing()) {
                    }
                    ...
                    e2.printStackTrace();
                    FLog.e(AvifDecoderUtil.TAG, "decode avif error because read data from encodedImage failed", e2.getMessage());
                    if (FrescoSystrace.isTracing()) {
                        FrescoSystrace.endSection();
                    }
                    return null;
