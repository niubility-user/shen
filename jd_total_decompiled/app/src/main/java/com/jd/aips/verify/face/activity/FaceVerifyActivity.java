...
public abstract class FaceVerifyActivity extends AppCompatActivity implements FaceDetector.DetectCallback, Camera.PreviewCallback {
    ...
    */
    protected int getActionDetectTime(int i2, FaceSdk.Config config) {
        int parseInt;
        switch (i2) {
            case 1002:
                parseInt = Integer.parseInt(config.face_action_timeout_1002);
                break;
            case 1003:
                parseInt = Integer.parseInt(config.face_action_timeout_1003);
                break;
            case 1004:
                parseInt = Integer.parseInt(config.face_action_timeout_1004);
                break;
            case 1005:
                parseInt = Integer.parseInt(config.face_action_timeout_1005);
                break;
            default:
                return 10;
        }
    ...
    public void onPreviewFrame(byte[] bArr, Camera camera) {
        ...
        if (this.verifyState.compareAndSet(1, 2)) {
            ...
            if (isDetecting()) {
                try {
                    toDetect(bArr, i2, i3);
                    if (this.reportEnable && (this.lastReportTime == -1 || SystemClock.elapsedRealtime() - this.lastReportTime > this.reportInterval * 1000)) {
                        Handler handler = this.workHandler;
                        handler.sendMessage(handler.obtainMessage(100, i2, i3, bArr));
