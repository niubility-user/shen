...
public class ImageTextPopView implements IPopView {
    ...
    public View initPopView(Context context, MessagePopModel messagePopModel, View.OnClickListener onClickListener) {
        View inflate = LayoutInflater.from(context).inflate(R.layout.message_pop_item_photo, (ViewGroup) null);
        ...
        if (!TextUtils.isEmpty(messagePopModel.stationSImgPath)) {
            JDImageUtils.displayImage(messagePopModel.stationSImgPath, (SimpleDraweeView) inflate.findViewById(R.id.message_pop_left_image));
        }
