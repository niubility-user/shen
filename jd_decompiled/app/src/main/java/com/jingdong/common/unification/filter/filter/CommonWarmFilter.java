...
public class CommonWarmFilter extends CommonFilterGroup {
    private Map<String, CommonFilter> filters;

    public CommonWarmFilter() {
        ...
        CommonRGBFilter commonRGBFilter = new CommonRGBFilter(1.2f, 1.04f, 1.0f);
    ...
    public CommonWarmFilter(int i2) {
        addFilter(new CommonSharpenFilter(FilterTools.range(i2, 0.0f, 0.2f)));
        addFilter(new CommonRGBFilter(FilterTools.range(i2, 1.0f, 1.2f), FilterTools.range(i2, 1.0f, 1.04f), 1.0f));
