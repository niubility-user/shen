...
public class TemplateSevenTeenPopView implements IPopView {
    ...
    public View initPopView(Context context, MessagePopModel messagePopModel, View.OnClickListener onClickListener) {
        View inflate = LayoutInflater.from(context).inflate(R.layout.message_pop_item_template_seventeen, (ViewGroup) null);
        ...
        try {
            ...
            JDImageUtils.displayImage(messagePopModel.stationSImgPath, (SimpleDraweeView) inflate.findViewById(R.id.message_pop_left_image), jDDisplayImageOptions.showImageOnLoading(i2).showImageOnFail(i2).showImageForEmptyUri(i2));
        } catch (Exception e2) {
            e2.printStackTrace();
        }
        ...
        if (!TextUtils.isEmpty(messagePopModel.alert)) {
            ...
            textView2.setTextColor(DeepDarkUtils.getInstance().getDarkColorFromJson(JDDarkUtil.COLOR_808080));
        }
        if (!TextUtils.isEmpty(messagePopModel.btnText)) {
            TextView textView3 = (TextView) inflate.findViewById(R.id.message_pop_right_bt);
            textView3.setText(messagePopModel.btnText);
            textView3.setBackground(context.getResources().getDrawable(DeepDarkChangeManager.getInstance().getUIMode() == 0 ? R.drawable.message_pop_lotter_bt_bg : R.drawable.message_pop_lotter_bt_bg_dark));
        }
        return inflate;
    }
}
