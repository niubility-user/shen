...
public class ByteBufferGifDecoder implements ResourceDecoder<ByteBuffer, GifDrawable> {
    ...
    private static int getSampleSize(GifHeader gifHeader, int i2, int i3) {
        ...
        if (Log.isLoggable(TAG, 2) && max > 1) {
            String str = "Downsampling GIF, sampleSize: " + max + ", target dimens: [" + i2 + JshopConst.JSHOP_PROMOTIO_X + i3 + "], actual dimens: [" + gifHeader.getWidth() + JshopConst.JSHOP_PROMOTIO_X + gifHeader.getHeight() + "]";
        }
        return max;
    }
